...
internal static class ClientGameLauncher
{
    ...
    static void DoRun(string[] args)
    {
        ...
        string[] mainArgs = new string[] { "-console" }.Concat(args).ToArray();//   args; 
        if (IsServer)
            mainArgs = new string[] { "-server" }.Concat(args).ToArray();
    ...
    static void ApplyDetours()
    {
        ...
        detours.Add(new Hook(tpt.GetMethod("ForceStaticInitializers", fstatic, new Type[] { typeof(Assembly) })!,
