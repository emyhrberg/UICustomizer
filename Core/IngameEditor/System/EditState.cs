...
namespace UICustomizer.Core.IngameEditor.System
{
    public class EditState : UIState
    {
        ...
        public void PopulateDefaultColors()
        {
            ...
            foreach (Element ele in Enum.GetValues<Element>())
            {
                elementColors[ele] = ele switch
                {
                    ...
                    _ => Color.Red // default to red for unknown element
                };
            }
        }
        ...
        public override void Draw(SpriteBatch sb)
        {
            ...
            if (!ModContent.GetInstance<EditSystem>().Enabled) return;
            DrawHitboxes(sb);
        }

        private void DrawHitboxes(SpriteBatch sb)
        {
            // Draw hitbox around every element
        ...
        public static void DrawHitboxOutlineAndText(SpriteBatch sb, Rectangle rect, Element element, int x = 0, int y = 0, Color color = default)
        {
            ...
            if (EditorFlags.ShowHitboxes)
            {
                ...
                sb.Draw(TextureAssets.MagicPixel.Value, fillRect, color * 0f);
